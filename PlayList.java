...
class PlayList {
    ...
    public boolean add(Track track) {
        if (size >= maxSize)// in case its bigger
        {
            return false;
        }
        tracks[size] = track;
    ...
    private int minIndex(int start) {
        if (start < size - 1 && size >= 0) {
            int minIndex = start;
            int minNumber = tracks[start].getDuration();
            for (int i = start + 1; i < size; i++) {
                if (tracks[i].getDuration() < minNumber) {
                    minNumber = tracks[i].getDuration();
                    minIndex = i;
                }
            }
            return minIndex;
